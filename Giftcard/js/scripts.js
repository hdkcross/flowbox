...
var should_value_localstorage = function(){
    var should_value;
    var current_page = $$('body').get('id');
    if(current_page == 'page_doi'){
        should_value = window.location.href.split('sc/')[1];
    }
    else{
        should_value = $$('#piu').get('value')[0];
    }
    return should_value;
};
var popmaster = (function() {
    ...
    function get_pop_options(pop) {
        var pop_options = {
            ...
            shouldFire: function(should, FMiKauYpzhc_object) {
                ...
                if( localStorage_is_supported && should_fire){
                    var page_info =  $$('body').get('id') + '_' + FMiKauYpzhc_object.name + '_' + $$('#wingame_pk').get('value') +  + $$('#freetest_pk').get('value');

                    if(localStorage.getItem('BetterJs' + page_info.hash()) === should_value_localstorage()){
                        should_fire = false;
                    }
                    else{
                        should_fire = true;
                    }
                    remove_matched_from_localstorage('BetterJs', 'BetterJs' + page_info.hash());
                }

                if(should_fire === true &&  pop.has_been_fired === false) {
    ...
    function trigger_clicked(FMiKauYpzhc_object) {
        ...
        if(typeOf(FMiKauYpzhc.getClickedElement()) == 'element'){
            pop_under_objects[FMiKauYpzhc_object.id].trigger.each(function(trigger) {
                ...
                FMiKauYpzhc.Logger.log(trigger);
                FMiKauYpzhc.Logger.log(FMiKauYpzhc.getClickedElement());
                FMiKauYpzhc.Logger.log(trigger === FMiKauYpzhc.getClickedElement());
                FMiKauYpzhc.Logger.log(trigger.contains(FMiKauYpzhc.getClickedElement()) === true);
    ...
    function get_form_data() {
        ...
        if($(document.body).get('id') && $(document.body).get('id') != 'page_doi'){
            if(data.length == 0){
                data += 'no_val=1';
            } else {
                data += '&no_val=1';
